...
namespace CoffeeCafeProject
{
    public partial class FrmMenu : Form
    {
        ...
        private void getallMenuToListView()
        {
            ...
            string connectionstring = @"server=LAPTOP-G9D396R2\SQLEXPRESS;Database=coffee_cafe_db;Trusted_Connection=True;";
            ...
            else
            {
                ...
                using (SqlConnection sqlConnection = new SqlConnection(connectionstring))
                {
                    try
                    {
                        ...
                        //ก่อนจะบันทึกให้ตรวจสอบก่อนว่ามีเมนูอยู่แล้ว 10 เมนูหรือยัง ถ้ามี 10 เมนูแล้ว ให้แสดงข้อความเตือนผู้ใช้บันทึกไม่ได้
                        //ต้องเอาของเก่าออกก่อน
                        string countSQL = "SELECT COUNT(*) FROM menu_tb";
                        using (SqlCommand command = new SqlCommand(countSQL, sqlConnection))
                        {
                        ...
                        SqlTransaction SqlTransaction = sqlConnection.BeginTransaction(IsolationLevel.ReadCommitted); //.ใช้กับ Insert/upate/delete
        ...
        {
            //เอาข้อมูลของรายการที่เลือกไปแสดงที่หน้าจอ แล้วปุ้มบันทึกใช้ไม่ได้ แก้ไขกับลบใช้ได้
            ...
            //เอารูปจาก ListView มาแสดงที่ Picture Box
